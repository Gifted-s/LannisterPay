module.exports = function (tokens, fcsConfigNumber) {
  ...
  if (tokens[5] !== 'APPLY') {
    return { error: `[Syntax Error]: Expexted token after ${tokens[0]}  ${tokens[1]} ${tokens[2]}  ${tokens[3]} ${tokens[4]} to be APPLY at at Fee Configuration Spec: ${fcsConfigNumber}  ` };
  }
  if (!tokens[6]) {
  ...
  if (tokens[6] === 'PERC') {
    if (!parseFloat(tokens[7]) && parseFloat(tokens[7]) !== 0) {
      return { error: `FEE-VALUE is not represented as a Float at Fee Configuration Spec: ${fcsConfigNumber} , please check FeeConfigurationSpec and try again` };
    }
  } else if (tokens[6] === 'FLAT') {
    if (!parseInt(tokens[7]) && parseInt(tokens[7]) !== 0) {
      return { error: `FEE-VALUE is not represented as an Integer at Fee Configuration Spec: ${fcsConfigNumber} , please check FeeConfigurationSpec and try again` };
    }
  } else if (tokens[6] === 'FLAT_PERC') {
    const feeValueToArray = tokens[7].split(':');
    if (!parseInt(feeValueToArray[0]) && parseInt(feeValueToArray[0]) !== 0) {
      return { error: `FEE-VALUE must contain an Integer in this format [int:float] at Fee Configuration Spec: ${fcsConfigNumber} , please check FeeConfigurationSpec and try again` };
    }
    if (!parseFloat(feeValueToArray[1]) && parseFloat(feeValueToArray[1]) !== 0) {
      return { error: `FEE-VALUE must contain a Float in this format [int:float]  at Fee Configuration Spec: ${fcsConfigNumber} , please check FeeConfigurationSpec and try again` };
    }
  } else {
